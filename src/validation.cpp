...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams, int nHeight)
{
    block.SetNull();
    ...
    // Check the header
    if (!CheckProofOfWork(block.GetHashPoW(nHeight, consensusParams), block.nBits, consensusParams))
            ...
            try {
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        if (ReadBlockFromDisk(*pblockrecursive, it->second, chainparams.GetConsensus(), nHeight))
                        {
